...
予約がひとつでも残っている間、エディタの上にカウントダウンと小さなアニメーションが
表示されます。犬か猫のどちらかで、カウントダウンが始まるたびにランダムで選ばれます：

```
⏱ keep going in 7m 58s (14:23)
```

アートはオリジナルのピクセルアートで、truecolor のハーフブロックで描画しています
（端末の 1 セルに 2 ピクセル）。色深度は自動でフォールバックします。端末が対応して
いれば 24 ビット、そうでなければ 256 色、色が使えなければフラットな ASCII アートです。
フレームは 900ms ごとに進み、タイマーは予約がある間だけ存在するので、アイドル中の
セッションには一切影響しません。
