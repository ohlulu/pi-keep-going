...
4. **usage-limit 偵測雙通道**：
   ...
   - **Gotcha（2026-07-11 事故，session 019f51c4-…3435）**：通道 A 對 Anthropic 實際上永遠不會觸發 —— pi-ai 的呼叫序是 `await client.messages.create(...).asResponse()` 之後才呼叫 `onResponse`，而 Anthropic SDK 在非 2xx 時先 throw（`makeStatusError`），所以 429 從不進 `after_provider_response`，`cached429` 恆為 null；且 `rate_limit_error` body 不含 reset 時間。因此 auto-resume 路徑上「主動打 usage API」的 fallback 是**必要通道**而非最後手段——首版漏接（只接在 `/kg auto`），5h limit 時必然退化成 notify。已修：`agent_settled` 偵測到 usage limit 且 reset 為 null 時 fallback 打 usage API（帶 generation guard），regression test 見 `test/auto-resume.test.ts`。
